...
```text
这个项目很小，不用 CodeGraph，直接读相关文件修改。
```

陌生项目：

```text
先用 CodeGraph 做项目 onboarding，告诉我入口、核心模块、测试和构建方式，不要先改代码。
```

核心功能修改：

```text
先用 CodeGraph 查影响范围，再读关键源码，确认方案后再改。
```

复杂 bug：

```text
用 CodeGraph 找调用链和相关模块，再定位 bug 根因。
```

代码审查：

```text
用 CodeGraph 做结构级 review，重点看调用关系、影响范围、缺测试和高风险模块。
```
